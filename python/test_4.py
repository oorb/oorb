...
def unpack_oorbArray(orbits):
    ...
    orbits = pd.DataFrame(oorbArray, columns=['objId', 'q', 'e', 'i', 'node', 'argperi', 't_p', 'elem_type', 't_0', 'epoch_type',
                                              'H', 'gval'])
    return orbits
...
if __name__ == "__main__":
    ...
    newepoch = 49385
    command = "/bin/rm test_orbits_new.des ; oorb --task=propagation --orb-in=test_orbits.des --orb-out=test_orbits_new.des --epoch-mjd-tt=%f" % newepoch
    print command
